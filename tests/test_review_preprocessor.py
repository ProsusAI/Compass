...
class TestPopulateSmsEmoaReviewFields:
    """Tests for _populate_sms_emoa_review_fields and its wiring in build_review_briefing."""

    def _make_sms_emoa_state(
        self,
        *,
        population: list[dict] | None = None,
        hypervolume_history: list[float] | None = None,
        stagnation_window: int = 5,
        elite_versions: list[str] | None = None,
    ) -> SearchState:
        """Build a minimal SMS-EMOA SearchState."""
        elite = [
            Candidate(
                prompt_version=v,
                parent_version=None,
            ...
            for v in (elite_versions or [])
        ]
        algorithm_state: dict[str, Any] = {}
        if population is not None:
            algorithm_state["population"] = population
        if hypervolume_history is not None:
            algorithm_state["hypervolume_history"] = hypervolume_history
        algorithm_state["stagnation_window"] = stagnation_window
    ...
    def _make_briefing(self, state: SearchState) -> Any:
        score_reports = {
            c.prompt_version: _make_report_dict(accuracy=0.80, cost=1.0)
        ...
        return build_review_briefing(
            ...
            score_reports=score_reports,
            historical_reports={},
            prompt_texts={c.prompt_version: f"prompt {c.prompt_version}" for c in state.elite_set},
            directive_history=[],
            candidate_versions=[],
            parent_versions={},
        )

    def test_empty_population_both_parents_none(self) -> None:
        """When both algorithm_state.population and elite_set are empty, parent versions are None."""
        state = self._make_sms_emoa_state(population=[], elite_versions=[])
        briefing = self._make_briefing(state)
        assert briefing.parent_a_version is None
        assert briefing.parent_b_version is None

    def test_single_member_population_both_parents_same(self) -> None:
        """With exactly 1 member, both parent_a and parent_b resolve to that version."""
        state = self._make_sms_emoa_state(
            population=[
                {
                    "prompt_version": "v1",
                    "parent_version": None,
        ...
        briefing = self._make_briefing(state)
        assert briefing.parent_a_version == "v1"
        assert briefing.parent_b_version == "v1"

    def test_two_member_population_distinct_parents(self) -> None:
        """With ≥2 members, parent_a and parent_b are distinct and both in the population."""
        state = self._make_sms_emoa_state(
            population=[
                {
                    "prompt_version": "v1",
        ...
        briefing = self._make_briefing(state)
        assert briefing.parent_a_version != briefing.parent_b_version
        assert briefing.parent_a_version in {"v1", "v2"}
        assert briefing.parent_b_version in {"v1", "v2"}

    def test_fallback_to_elite_set_when_population_empty(self) -> None:
        """Empty pocket population falls back to elite_set for parent sampling."""
        state = self._make_sms_emoa_state(
            population=[],
            elite_versions=["va", "vb"],
        )
        briefing = self._make_briefing(state)
        assert briefing.parent_a_version in {"va", "vb"}
        assert briefing.parent_b_version in {"va", "vb"}
        assert briefing.parent_a_version != briefing.parent_b_version

    def test_stagnation_signal_hypervolume_history_passthrough(self) -> None:
        """stagnation_signal contains hypervolume_history and stagnation_window from pocket."""
        state = self._make_sms_emoa_state(
            hypervolume_history=[0.1, 0.2, 0.35],
            stagnation_window=7,
        )
        briefing = self._make_briefing(state)
        assert briefing.stagnation_signal is not None
        assert briefing.stagnation_signal["hypervolume_history"] == [0.1, 0.2, 0.35]
        assert briefing.stagnation_signal["stagnation_window"] == 7

    def test_stagnation_signal_defaults_when_absent(self) -> None:
        """Absent hypervolume_history defaults to empty list; stagnation_window defaults to 5."""
        state = self._make_sms_emoa_state()
        briefing = self._make_briefing(state)
        assert briefing.stagnation_signal is not None
        assert briefing.stagnation_signal["hypervolume_history"] == []
        assert briefing.stagnation_signal["stagnation_window"] == 5

    def test_beam_fields_none_for_sms_emoa(self) -> None:
        """Beam-specific fields are None for SMS-EMOA runs."""
        state = self._make_sms_emoa_state()
        briefing = self._make_briefing(state)
        assert briefing.beam_rank is None
        assert briefing.crowding_distance is None
