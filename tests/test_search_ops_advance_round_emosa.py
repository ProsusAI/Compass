...
class TestAdvanceRoundEmosaSearch:
    ...
    def test_neighborhood_replacement(self, tmp_path: Path) -> None:
        ...
        # With neighborhood_size=4 (explicitly set above) but EMOSA_REPLACEMENT_CAP=2,
        # child_v1 (0.92, 0.01) is highly dominant but may replace at most CAP=2 neighbors.
        neighbor_ids = {1, 2, 3, 4}
        replaced = [
            t for t in pocket.trajectories if t.trajectory_id in neighbor_ids and t.current_solution == "child_v1"
        ]
        assert len(replaced) >= 1, "Expected at least one neighbor to be replaced"
        assert len(replaced) <= EMOSA_REPLACEMENT_CAP, (
            f"Expected at most {EMOSA_REPLACEMENT_CAP} neighbors replaced (cap), got {len(replaced)}"
        )
    ...
    def test_advance_emosa_search_no_full_collapse_on_corner_dominator(self, tmp_path: Path) -> None:
        """Corner-dominating child must not sweep all K trajectories (regression for run 97903179).

        Build K=5 trajectories each with a distinct current_solution.  Inject 5
        pending children — one is a "corner dominator" (norm_q=0, sitting at
        ideal_point[0]) parented on T0's current_solution.  After advance_round_emosa:

        - T0 may accept the corner dominator via Metropolis (it's better for T0).
        - Neighborhood replacement is capped at EMOSA_REPLACEMENT_CAP neighbors.
        - Therefore at most EMOSA_REPLACEMENT_CAP + 1 trajectories can adopt it.
        - At least one of the originally-distinct trajectories must keep a different
          current_solution.
        """
        run_id = "emosa-no-collapse-corner"
        # K=5 trajectories, distinct seeds
        num_traj = 5
        # ideal_point and nadir_point set so the corner dominator has norm_q=0
        ideal = (0.95, 0.01)  # best quality = 0.95
        nadir = (0.50, 0.09)
        weight_vectors = compute_weight_vectors(num_traj)

        trajectories: list[TrajectoryState] = []
        for i in range(num_traj):
            q = 0.70 - i * 0.03  # distinct quality per trajectory
            c = 0.05 + i * 0.01
            e = compute_tchebycheff_energy(q, c, weight_vectors[i], ideal, nadir)
            trajectories.append(
                TrajectoryState(
                    trajectory_id=i,
                    weight_vector=weight_vectors[i],
                    current_solution=f"distinct_seed_v{i}",
                    current_quality=q,
                    current_cost=c,
                    current_energy=e,
                    acceptance_history=[True],
                    temperature=100.0,  # Very high T → Metropolis accepts almost anything
                    alpha=0.95,
                    step_count=1,
                )
            )
        ...
        annealing = AnnealingState(
            ...
            ideal_point=ideal,
            nadir_point=nadir,
            # Use default neighborhood_size=2 (canonical B << K)
        )
        _init_emosa_with_annealing(run_id=run_id, output_dir=tmp_path, annealing=annealing)

        # Corner dominator: quality == ideal_point[0], so norm_q = 0.
        # Under every weight vector this has the minimal possible Tchebycheff component
        # on the quality axis, making it highly attractive across all trajectories.
        corner_dominator = Candidate(
            prompt_version="corner_dominator",
            parent_version="distinct_seed_v0",  # parented on T0
            quality_score=ideal[0],  # = 0.95, at ideal → norm_q = 0
            cost=0.03,
            round_introduced=2,
            eval_status="complete",
        )
        # Non-dominating children for the other trajectories
        other_children = [
            Candidate(
                prompt_version=f"ordinary_child_v{i}",
                parent_version=f"distinct_seed_v{i}",
                quality_score=0.68 - i * 0.03,
                cost=0.06 + i * 0.01,
                round_introduced=2,
                eval_status="complete",
            )
        ...
        _save_pending(run_id, [corner_dominator, *other_children], tmp_path)
        ...
        # Count how many trajectories ended up with the corner dominator
        dominator_count = sum(1 for t in pocket.trajectories if t.current_solution == "corner_dominator")

        # T0 may accept via Metropolis; at most EMOSA_REPLACEMENT_CAP others can be
        # replaced via neighborhood replacement → total ≤ CAP + 1.
        assert dominator_count <= EMOSA_REPLACEMENT_CAP + 1, (
            f"Corner dominator swept {dominator_count} trajectories; "
            f"expected at most {EMOSA_REPLACEMENT_CAP + 1} (originator + cap)."
        )
        # At least one trajectory should retain a different current_solution
        distinct_solutions = {t.current_solution for t in pocket.trajectories}
        assert len(distinct_solutions) >= 2, (
            f"All trajectories collapsed to the same solution: {distinct_solutions}. "
            "Expected at least 2 distinct solutions after one round."
        )
