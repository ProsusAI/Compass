...
from odysseus.agents.pipeline_guards import require_artifacts, check_artifacts
...
class TestRequireArtifacts:
    def test_passes_when_all_exist(self, tmp_path: Path) -> None:
        ...
        @require_artifacts(tmp_path / "a.json", tmp_path / "b.json",
                          stage=2, stage_name="Data Validated", hint="Run validate_dataset first.")
        def my_tool() -> str:
            return "ok"
        assert my_tool() == "ok"
    ...
    def test_raises_when_missing(self, tmp_path: Path) -> None:
        ...
        @require_artifacts(tmp_path / "a.json", tmp_path / "missing.json",
                          stage=2, stage_name="Data Validated", hint="Run validate_dataset first.")
        def my_tool() -> str:
            return "ok"
    ...
    def test_error_includes_stage_hint_and_status_ref(self, tmp_path: Path) -> None:
        @require_artifacts(tmp_path / "missing.json",
                          stage=3, stage_name="Routing Analysis", hint="Complete phases 1-3 first.")
        def my_tool() -> str:
            return "ok"
    ...
    async def test_works_with_async(self, tmp_path: Path) -> None:
        ...
        @require_artifacts(tmp_path / "a.json",
                          stage=1, stage_name="Input", hint="Submit report.")
