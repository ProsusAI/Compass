...
class TestStrategySeams:
    ...
    def test_sms_emoa_state_yields_full_chips(self, tmp_path: Path) -> None:
        """A full SMS-EMOA state (algorithm_state pocket) yields 4 chips."""
        search_dir = tmp_path / "search"

        archive = [_make_candidate("cv-0-0", None, 0)]
        state = {
            "search_state_id": "sms-test",
            "backend": "anthropic",
            "iteration": 3,
            "algorithm": "sms_emoa",
            "algorithm_state": {
                ...
                "evaluations_used": 12,
                "evaluation_budget": 50,
                "hypervolume_history": [0.1, 0.2, 0.3],
            },
            "elite_set": archive,
            "warm_up_complete": True,
            "evaluation_budget": 50,
        ...
        assert data["strategy_label"] == "SMS-EMOA"
        chips = data["algorithm_chips"]
        assert len(chips) == 4
        labels = [c["label"] for c in chips]
        assert "μ" in labels
        assert "iter" in labels
        assert "evals" in labels
        assert "HV" in labels
        # Verify specific values
        mu_chip = next(c for c in chips if c["label"] == "μ")
        assert mu_chip["value"] == "8"
        hv_chip = next(c for c in chips if c["label"] == "HV")
        assert hv_chip["value"] == "0.3000"
