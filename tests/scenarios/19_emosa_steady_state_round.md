...
- MCP tools: `get_pipeline_status`, `get_search_state_tool`, `build_review_briefing_tool`,
  `register_candidate_tool`, `run_batch_eval`, `advance_step_tool`,
  `save_trajectory_child_variants_tool`, `record_directive_outcomes_tool`
...
1. **Per-fork review (K=5)**: The pipeline dispatches five parallel Review Agent calls, one
   per trajectory. Each call invokes `build_review_briefing_tool` with `trajectory_id=t`
...
2. **Build (K=5)**: The Prompt Builder registers one child candidate per trajectory via
   `register_candidate_tool`, producing `v_child_0` through `v_child_4` with
   `parent_version=v_seed_t`.

3. **Batch evaluation**: `run_batch_eval` scores all 5 children concurrently.

4. **Advance step**: `advance_step_tool` processes the scored children through the full
   steady-state loop:
...
- After `advance_step_tool`, `step_count == 2` and `temperature ≈ T_initial * alpha^2 = 0.9025`.
...
- Expected flow:
  ...
  2. For each `t in [0, 1, 2, 3, 4]`:
     a. `build_review_briefing_tool(run_id=..., trajectory_id=t)` — per-fork briefing.
     b. Review Agent (or inline) emits one child directive targeting `v_seed_t`.
     c. `register_candidate_tool` registers `v_child_t` with `parent_version=v_seed_t`.
  3. `run_batch_eval` evaluates all 5 children.
  4. `advance_step_tool` advances state; verify `step_count=2`, `temperature≈0.9025`.

**Behaviour:**
1. Call `get_pipeline_status` to confirm `loop_phase="review"` and `algorithm="emosa"`.
2. For trajectories 0 through 4 in order:
   a. Call `build_review_briefing_tool` with `trajectory_id=<t>`.
   b. Confirm the returned briefing has `trajectory_id == t` and non-null `weight_vector`.
   c. Register one child candidate with `parent_version=v_seed_<t>` via `register_candidate_tool`.
3. Call `run_batch_eval` to score all 5 children.
4. Call `advance_step_tool`. Verify the result shows:
   - `algorithm_state.step_count == 2`
   - `algorithm_state.temperature ≈ 0.9025` (within 0.001)
   - No `NotImplementedError`
5. Call `get_search_state_tool` and verify each trajectory has `acceptance_history` with
...
call `build_review_briefing_tool` with `trajectory_id=<t>` and confirm it returns
trajectory-specific fields. After building and evaluating all 5 children, call
`advance_step_tool` and verify `step_count` increments to 2 and `temperature ≈ 0.9025`."
...
- [ ] `build_review_briefing_tool` is called 5 times, once with `trajectory_id=0`, once with
...
- [ ] Each returned briefing has a non-null `stagnation_signal` with `temperature` and
      `review_exit` keys
- [ ] No `NotImplementedError` is raised during any `build_review_briefing_tool` call
...
- [ ] No `advance_step_tool` call is made before all 5 children are scored

### Advance Step — Steady-State Update
- [ ] `advance_step_tool` is called exactly once after all 5 children are scored
- [ ] After `advance_step_tool`, `algorithm_state["step_count"] == 2`
- [ ] After `advance_step_tool`, `algorithm_state["temperature"]` is within 0.001 of
      `T_initial * alpha^2 = 1.0 * 0.95^2 = 0.9025`
- [ ] After `advance_step_tool`, `algorithm_state["total_evals"] == 10`
- [ ] After `advance_step_tool`, `algorithm_state["phase"] == "search"` (not converged yet)
- [ ] No `NotImplementedError` raised during `advance_step_tool`
...
- [ ] Elite set (archive) size is ≥ 1 after `advance_step_tool`
...
- [ ] Top-level `loop_phase` after `advance_step_tool` is `"review"` (next round ready)
