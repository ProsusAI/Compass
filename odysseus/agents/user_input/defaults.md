...
|---|---|---|---|
| `target_metrics` | `["f1/macro"]` | F1 macro is a strong general-purpose metric for routing problems — it handles class imbalance well and reveals per-class performance, unlike accuracy which can mask poor routing on minority classes. | "No target metrics specified — defaulting to F1 macro average (`f1/macro`). You can specify metrics such as `accuracy >= 0.85` or `cost_change_with_overhead <= -0.30` in a follow-up." |
...
| `evaluation_budget` | `60` | Most routing problems need 50–70 prompt versions to converge; 60 is a balanced default. | "No evaluation budget provided — defaulting to 60 prompt versions." |
