...
from compass.agents.pipeline.guards import check_artifacts  # noqa: F401
from compass.agents.pipeline.status import get_pipeline_status as _get_pipeline_status
...
from compass.mcp.prompts import _STAGE_PROMPT_BODIES  # noqa: E402
...
async def optimize_routing_prompt(ctx: Context) -> str:
    """Start the Compass routing prompt optimization pipeline.
    ...
    """
    try:
        system_prompt = _load_text("compass/agents/prompts/user_input_system.md")
    ...
    return (
        f"<pipeline_status>\n{status_json}\n</pipeline_status>\n\n"
        f"<instructions>\n"
        f"You are the Compass pipeline orchestrator. You have two modes:\n\n"
...
async def start_stage(ctx: Context, run_id: str | None = None) -> str:
    ...
    if current_stage_num == 1:
        ...
        if activate_prompt == "compass_review_agent_cold_start":
            stage = "review_cold"
    ...
    try:
        if run_id is None:
            ...
            if activate_prompt in _REVIEW_AGENT_PROMPT_NAMES:
                ...
                from compass.mcp.prompts import assemble_review_prompt

                if activate_prompt == "compass_review_agent_cold_start":
                    phase = "cold_start"
                elif activate_prompt == "compass_review_agent_post_coldstart":
