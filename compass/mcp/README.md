...
| [`prompt_building_tools.py`](prompt_building_tools.py) | `init_search_state` (no `algorithm`/`algorithm_state` — hardcoded per branch), `register_candidate`, `run_batch_eval`, `record_eval_result`, `advance_step`, `get_search_state`, `save_prompt`, `get_child_variants`, `get_edit_directives` |
| [`review_tools.py`](review_tools.py) | `build_review_briefing`, `record_directive_outcomes`, and Stage 4 review detail/query tools such as `query_dev_examples`, `query_holdout_examples`, `query_eval_results`, `get_score_report`, and `get_confusion_cell` |
...
| [`resources.py`](resources.py) | MCP resource definitions (`compass://agents/...`, `compass://backends/...`) |
| [`prompts.py`](prompts.py) | MCP prompt definitions (`compass_routing_input`, `compass_data_validation`, etc.) |
...
| `prompt_building` | `init_search_state`, `register_candidate`, `run_batch_eval`, `record_eval_result`, `advance_step`, `get_search_state`, `get_edit_directives`, `get_child_variants`, `save_prompt`, `get_pipeline_status` | All prompt-version evaluation goes through `run_batch_eval`; pass a single-element list for one-at-a-time algorithms. |
| `review_cold` | `build_review_briefing`, `record_directive_outcomes`, `query_dev_examples`, `query_eval_results`, `get_search_state`, `get_score_report`, `get_confusion_cell`, `get_round_child_variants`, `get_dataset_oracle_distribution`, `get_per_class_recall`, `get_pipeline_status` | Cold sub-agents may page dev rows by route or `example_ids`, and can inspect per-example eval rows without reading `results.jsonl`; still no `get_prompt_text` / `query_holdout_examples` |
| `review` | `build_review_briefing`, `record_directive_outcomes`, `query_dev_examples`, `query_holdout_examples`, `query_eval_results`, `get_prompt_text`, `get_search_state`, `get_score_report`, `get_confusion_cell`, `get_round_child_variants`, `get_dataset_oracle_distribution`, `get_per_class_recall`, `get_pipeline_status` | Steady-review toolbelt; dataset rows remain query-only, paginated, support targeted lookup by `example_ids`, and now expose joined per-example eval detail |
...
- `query_eval_results` returns markdown per-example eval detail joined against `analysis/dev.jsonl`, so review agents can inspect misroutes by `(true_route, predicted_route)` or `example_ids` without shelling out to `results.jsonl`.
...
- `build_final_report_briefing` still returns JSON, but `FinalReportBriefing` now carries additive markdown snippet dicts keyed by `prompt_version` (`dev_score_report_md`, `holdout_score_report_md`, `baseline_comparison_md`) so the Final Report agent can render every user-selected holdout candidate without re-rendering score reports itself.
...
|---|---|---|
| `compass_review_agent_iterative`, `compass_review_agent_cold_start` | strong | High-stakes synthesis |
...
Other runtimes should map the tier to their equivalent backend model. No Compass model defaults are changed.
...
3. If the tool belongs to a new stage, add a new key to `STAGE_REGISTRY` and update `compass/agents/pipeline/status.py` to reference the new stage name in the relevant `subagent_instruction`.
