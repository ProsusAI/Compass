...
The `compass.eval.backends` subpackage provides a three-class design for configuring and invoking LLM providers:
...
`compass/eval/backends/profile.py`
...
`compass/eval/backends/registry.py`
...
`compass/eval/backends/anthropic_backend.py`
...
`compass/eval/backends/openai_backend.py`
...
`compass/eval/backends/bedrock_backend.py`
...
Each backend normalises provider-specific token usage fields to Compass's disjoint `TokenUsage` model:
