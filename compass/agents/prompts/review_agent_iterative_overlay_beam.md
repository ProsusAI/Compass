...
**Loop phase.** `review`, round ≥ 3. (Round 2 is handled by the post-cold-start override; the orchestrator dispatches that phase separately.)
**Total child count per round.** `beam_width` (read from `briefing.beam_width`).

**Agent-chosen allocation.** Emit `beam_width` children total, distributed across 1, 2, or 3 elites at your discretion. Spend the budget on whichever elite-set members **look most promising for reaching the user's threshold** — or, once the threshold is met, **for moving past the oracle point**. Allocation modes:
