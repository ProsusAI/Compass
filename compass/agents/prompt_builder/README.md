# compass/agents/prompt_builder — Prompt Builder Subpackage
...
The subpackage is code-only; the Prompt Builder Agent (LLM-driven, system prompt in `compass/agents/prompts/`) calls these functions via MCP tools defined in `compass/mcp/prompt_building_tools.py`. Inputs are populated via the discovery sequence defined in `compass/agents/prompts/prompt_builder_system.md` — the agent uses MCP tools (`get_routing_context`, `get_search_state`, `get_child_variants`, `get_prompt_text`) rather than a context dict.
