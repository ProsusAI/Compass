...
from compass.agents.pipeline import paths, tool_registry
...
_STAGE_4_BUILD_ACTION_RECOVER: str = (
    ...
    "REQUIRED: activate prompt 'compass_prompt_builder' before calling any build tools."
)
...
_STAGE_4_PHASE_CONFIG: dict[str, tuple[str, list[str], list[str], str]] = {
    "cold_review": (
        ...
        "REQUIRED: activate prompt 'compass_review_agent_cold_start' before calling any review tools.",
    ...
    "review": (
        ...
        "REQUIRED: activate prompt 'compass_review_agent_iterative' before calling any review tools.",
...
def _ensure_stage4_search_state(run_dir: Path, project_dir: Path | None = None) -> None:
    ...
    # Deferred import: compass.agents.prompt_builder.search_ops imports
    # compass.agents.pipeline.paths at module level, which would create a
    # circular import if hoisted here (stage4.py is part of the pipeline package).
    from compass.agents.prompt_builder.search_ops import init_search_state
...
def _next_action_for_stage_4(
    ...
    if phase == "rerun":
        ...
        return (
            ...
            "REQUIRED: activate prompt 'compass_prompt_builder_rerun' before calling any build tools.",
            tool_registry.RERUN_TOOLS,
            ["compass_prompt_builder_rerun"],
            rerun_instr,
            algorithm,
        )
