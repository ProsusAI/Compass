...
Review dispatches rely on stage-scoped tool filtering (`STAGE_REGISTRY`) plus the review-agent system prompt's explicit data-access policy — there is no custom sub-agent type binding.
