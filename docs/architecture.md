...
Review dispatches add one more Claude Code binding: when the active stage is `review` or `review_cold`, `start_stage` also requires `subagent_type: "odysseus-review-agent"` so the dispatched sub-agent is constrained to Odysseus MCP tools only; see [`.claude/agents/odysseus-review-agent.md`](../.claude/agents/odysseus-review-agent.md). The agent definition ships as bundled package data (`odysseus/templates/claude/agents/odysseus-review-agent.md`) and is materialized into `.claude/agents/` by `odysseus init`.
