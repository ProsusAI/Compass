...
`ReviewBriefing` is the complete pre-processed input for the Review Agent LLM, containing `CandidateAnalysis` list, `DiversityMetrics`, `DiminishingReturns`, `OracleMetrics`, per-class recall, `UserTargetProgress` list (progress toward user-specified metric targets; each entry carries `source_version` — the single candidate version evaluated, all entries sharing the same value), `single_candidate_meets_all` flag (`true` when every target is met by the same candidate — the only safe condition for `LoopSignal{action="exit"}`), `BatchOutcome` list (linking child variants to their eval results), and `ChildVariant` list. Strategy-specific optional fields (all default `None` for non-beam runs): `beam_rank` (dict mapping prompt_version → non-dominated rank; always 0 for beam since all elite members are on a single Pareto front), `crowding_distance` (dict mapping prompt_version → crowding distance; `inf` serialised as `1e308`), `hypervolume` (float; current hypervolume of elite set), `reference_point` (2-tuple; HV reference point), `stagnation_signal` (dict; hill-climb shape `{"count", "limit", "mutation_mode"}` or beam shape `{"hypervolume_delta", "backtrack_threshold"}`). `ReviewResult` is the LLM output: `candidate_ranking`, `child_variants`, `promotion_decisions`, `loop_signal`, `regression_guards`, and `directive_history_update`. Persistence (directive history, child variants, round reports) lives in [`review/ops.py`](../odysseus/agents/review/ops.py). Child variants are persisted to `child_variants.json` via `record_directive_outcomes_tool` and retrieved by the Prompt Builder via `get_edit_directives_tool`.
