...
- [ ] **A4** — Confusion analysis + contrast pairs + signal filters. `ConfusionImpact`, contrast-pair directives, cell-exhaustion. Verify contrast_pair round-trip. **Coordinate with A7-followup**: the consumers of `target_progress` may shift in this session, which will inform whether deferred `856a85a` lands as per-target `source_version` + `single_candidate_meets_all` flag (option a) or as a different surfacing of per-target leaders (option b). Decide here before A7-followup.
...
- [ ] **A7-followup** — Best-per-target metrics for `target_progress` (deferred from A7 / source `856a85a`). Either (a) add per-target `source_version` and a `single_candidate_meets_all` flag plus prompt updates so the Review Agent can distinguish single-prompt success from cherry-picked mixes, or (b) keep globally-best semantics and design a different way to surface per-target leaders. Coordinate with A4 (confusion analysis + contrast pairs) since the consumers of `target_progress` may shift there.
