...
- [ ] **A7** — Tooling: `query_misrouted_examples`, pagination, MCP cleanup. Verify pagination smoke test. Also pull in from A6 reclassification: `1182b89` (introduces `query_holdout_examples_tool` + `get_prompt_text_tool`, briefing scaffolding refactor), `4f5c891` (run-specific prompt-dir fallback in those tools), and `df5b6c5` (decompose `review_result` into separate parameters — may instead belong in C2 if entangled with sms-emoa-specific review state).
