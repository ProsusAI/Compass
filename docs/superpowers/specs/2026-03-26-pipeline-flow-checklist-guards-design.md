...
|------|-----------------|
| `submit_input_report` | Returns `run_id` in response (generator) |
...
**Accepted limitation:** The `stratified_split_tool` guard checks `scratch/*/phase3_validated.json` with a glob, which cannot distinguish between runs on the same dataset. This is acceptable because Phase 3 validation is deterministic for a given dataset — if a checkpoint exists, it is valid for any run using that dataset.
